/* daemon.c generated by valac 0.25.3.3-7022, the Vala compiler
...
static void caribou_daemon_on_bus_acquired (CaribouDaemon* self, GDBusConnection* conn) {
	...
	__catch0_g_io_error:
	{
		...
		_inner_error_ = NULL;
		_tmp1_ = e->message;
		g_error ("daemon.vala:40: Could not register D-Bus service: %s", _tmp1_);
		_g_error_free0 (e);
	}
	__finally0:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
...
static void caribou_daemon_on_get_proxy_ready (CaribouDaemon* self, GObject* obj, GAsyncResult* res) {
	...
	__catch1_g_error:
	{
		...
		_tmp7_ = _tmp6_;
		g_error ("daemon.vala:48: %s", _tmp7_);
		_g_free0 (_tmp7_);
		_g_error_free0 (e);
	}
	__finally1:
	...
	__catch2_g_error:
	{
		...
		g_warning ("daemon.vala:54: can't register event listeners: %s", _tmp9_);
		_g_error_free0 (e);
	}
	__finally2:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
			...
			case ATSPI_ROLE_ENTRY:
			{
				...
				if (g_strcmp0 (_tmp14_, "object:state-changed:focused") == 0) {
					...
					if (_tmp16_ == 1) {
						...
						g_debug ("daemon.vala:97: enter text widget in %s", _tmp27_);
						...
						if (_tmp28_ == _tmp29_) {
							...
							g_debug ("daemon.vala:102: leave text widget in %s", _tmp38_);
					...
					g_warning ("daemon.vala:106: unknown focus event: %s", _tmp40_);
				}
				break;
			}
			default:
...
static void caribou_daemon_on_focus_ignore_error (CaribouDaemon* self, AtspiEvent* event) {
	...
	__catch3_g_error:
	{
		...
		g_warning ("daemon.vala:119: error in focus handler: %s", _tmp3_);
		_g_error_free0 (e);
	}
	__finally3:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
...
static void caribou_daemon_on_text_caret_moved (CaribouDaemon* self, AtspiEvent* event, GError** error) {
	...
	if (_tmp0_ == _tmp2_) {
		...
		g_debug ("daemon.vala:136: object:text-caret-moved in %s: %d %s", _tmp42_, _tmp44_, _tmp47_);
...
static void caribou_daemon_on_text_caret_moved_ignore_error (CaribouDaemon* self, AtspiEvent* event) {
	...
	__catch4_g_error:
	{
		...
		_tmp2_ = e;
		_tmp3_ = _tmp2_->message;
		g_warning ("daemon.vala:146: error in text caret movement handler: %s", _tmp3_);
		_g_error_free0 (e);
	}
	__finally4:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
...
void caribou_daemon_quit (CaribouDaemon* self) {
	...
	if (_tmp0_ != NULL) {
		...
		__catch5_g_io_error:
		{
			...
			g_warning ("daemon.vala:185: can't hide keyboard: %s", _tmp4_);
			_g_error_free0 (e);
		}
		__finally5:
		if (G_UNLIKELY (_inner_error_ != NULL)) {
		...
		__catch6_g_error:
		{
			...
			g_warning ("daemon.vala:191: can't deregister event listeners: %s", _tmp6_);
			_g_error_free0 (e);
		}
		__finally6:
		if (G_UNLIKELY (_inner_error_ != NULL)) {
...
gint _vala_main (gchar** args, int args_length1) {
	...
	_tmp0_ = _ ("- daemon listening accessibility events to launch on screen keyboard");
