...
static void ____lambda2__gtk_object_destroy (GtkObject* _sender, gpointer self);
...
static GdkFilterReturn caribou_gtk_module_event_filter (CaribouGtkModule* self, GdkXEvent* xevent, GdkEvent* evt) {
	...
	_tmp12_ = gdkwindow;
	gdk_window_get_user_data (_tmp12_, &_tmp13_);
	data = _tmp13_;
	_tmp15_ = data;
	if (_tmp15_ == NULL) {
		_tmp14_ = TRUE;
		...
		void* _tmp16_;
		_tmp16_ = data;
		_tmp14_ = !G_TYPE_CHECK_INSTANCE_TYPE (_tmp16_, GTK_TYPE_WINDOW);
	}
	_tmp17_ = _tmp14_;
	if (_tmp17_) {
		result = GDK_FILTER_CONTINUE;
	...
	if (!((gboolean) ((gintptr) _tmp22_))) {
		...
		g_hash_table_insert (_tmp23_, _tmp25_, (gpointer) ((gintptr) TRUE));
		_tmp26_ = _data1_->window;
		g_signal_connect ((GObject*) _tmp26_, "notify::has-toplevel-focus", (GCallback) _caribou_gtk_module_toplevel_focus_changed_g_object_notify, self);
		_tmp27_ = _data1_->window;
		g_signal_connect (_tmp27_, "set-focus", (GCallback) _caribou_gtk_module_window_focus_changed_gtk_window_set_focus, self);
		_tmp28_ = _data1_->window;
		g_signal_connect_data ((GtkObject*) _tmp28_, "destroy", (GCallback) ____lambda2__gtk_object_destroy, block1_data_ref (_data1_), (GClosureNotify) block1_data_unref, 0);
	}
	result = GDK_FILTER_CONTINUE;
	_g_object_unref0 (gdkwindow);
	block1_data_unref (_data1_);
